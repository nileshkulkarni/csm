...
class CSPTester(test_utils.Tester):
    ...
    def predict(self,):
        ...
        if self.mask_preds is not None:
    ...
    def map_kp_img1_to_img2_robust(self, vis_inds, kps1, kps2, uv_map1, uv_map2, mask1, mask2):
        ...
        if True: # map uv points to the 3D template
        ...
        if True:
            ...
            min_dist = min_dist.view(-1)
            kp_transfer_error = torch.norm((transfer_kps.float() - kps2[:, 0:2]), dim=1)
            tfs_dist_error = torch.stack([kp_transfer_error, kp_mask, min_dist],dim=1)
           
            # transfer_min_dist = 


        return transfer_kps, tfs_dist_error
    ...
    def map_kp_img1_to_img2(self, vis_inds, kps1, kps2, uv_map1, uv_map2, mask1, mask2):
        ...
        transfer_kps = torch.stack(
            [min_indices % img_W, min_indices // img_W], dim=1)
        kp_transfer_error = torch.norm(kp_mask[:, None] * (transfer_kps.float() - kps2[:, 0:2]), dim=1)
    ...
    def evaluate_m1(self,):
        ...
        if self.opts.robust:
            transfer_kps12, tfs12_dist_err = self.map_kp_img1_to_img2_robust(
                common_kp_indices, kps_ind[0], kps_ind[1], uv_map[0], uv_map[1], mask[0], mask[1])
            transfer_kps21, tfs21_dist_err = self.map_kp_img1_to_img2_robust(
            common_kp_indices, kps_ind[1], kps_ind[0], uv_map[1], uv_map[0], mask[1], mask[0])
        return visutil.torch2numpy(transfer_kps12), visutil.torch2numpy(tfs12_dist_err), visutil.torch2numpy(transfer_kps21), visutil.torch2numpy(tfs21_dist_err)
    ...
    def test(self,):
        ...
        result_path = osp.join(
            opts.results_dir, 'results_vis_{}.mat'.format(n_iter))
        ...
        if not osp.exists(result_path) or opts.force_run:
            ...
            for i, batch in enumerate(izip(self.dl_img1, self.dl_img2)):
                ...
                transfer_kps12, tfs_dist12, transfer_kps21, tfs_dist21 = self.evaluate_m1()
                ...
                bench_stats_m1['kps_err'].append(tfs_dist12)
                bench_stats_m1['kps_err'].append(tfs_dist21)
